...
signale ce qui doit être facturé, et prépare le corps des abonnements de facturation Pennylane.
Le cabinet saisit les baux une fois ; le reste se met à jour d'une commande.
...
```
   ...
   │                                                                                action)                 │
   │                                                              └─►  Pennylane (aperçu JSON par bail)     │
   └────────────────────────────────────────────────────────────────────────────────────────────────────────┘
                                                                                         │
                                                                                         ▼ pennylane <société> [--push]
                                                             out/*.json (dry-run)  ──►  POST /billing_subscriptions
```
...
| Rafraîchissement | ✔ | Relit la feuille Baux (saisies du cabinet), régénère les feuilles calculées, conserve « Appliqué ? / Date / Commentaire », crée un `.bak`. |
| Contre-épreuve | ✔ | Un moteur Python indépendant recalcule tout ; un test recalcule le classeur avec LibreOffice et compare cellule à cellule (30 échéances de démo, 0 écart). |
...
| Révisions | Une ligne par bail × échéance : trimestres de référence, indices, coefficient, loyer brut, plafond, loyer retenu, mensuel, par échéance, TVA, TTC, **statut** | script ; cabinet pour Appliqué ?/Date/Commentaire |
| Alertes | Par bail : loyer actuel, prochaine révision, trimestre attendu, indice publié ?, révisions calculables non facturées, **action** | script |
...
```
indexation_loyer/   insee.py (API + cache) · periodes.py · baux.py · calcul.py (moteur) · workbook.py (Excel) · pennylane.py · cli.py · demo.py
config/             series_insee.json · pennylane_mapping.json · baux_modele.csv
...
tests/              14 tests, dont la parité LibreOffice ↔ moteur Python
